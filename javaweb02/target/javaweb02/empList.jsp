...
<body>
  ...
  %>
  <center>
  <a href="/javaweb02/emp?method=toAdd">新增用户</a><br>
    <table border="1">
      <tr>
        <th>员工编号</th>
        <th>员工姓名</th>
      ...
      <c:forEach items="${empList}" var="item">
        <tr>
          ...
          <th>${item.COMM}</th>
          <th>${item.deptno}</th>
          <th><a href="/javaweb02/emp?method=delete&empno=${item.getEmpno()}">删除</a>  
              <a href="/javaweb02/emp?method=toUpdate&empno=${item.getEmpno()}">修改</a></th>
        </tr>
      </c:forEach>
    </table>
...
function load(currentPage,pageSize) {
  window.location.href='/javaweb02/emp?method=selectAll&currentPage='+currentPage+'&pageSize='+pageSize
}
</script>
</html> 