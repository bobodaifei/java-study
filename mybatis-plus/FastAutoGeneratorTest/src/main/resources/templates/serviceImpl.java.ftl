...
@Override
@Transactional(rollbackFor = Exception.class)
public void modify${entity}(Long id, ${entity}DTO dto){
${entity} entity = INSTANCE.toEntity(dto);
...
@Override
public Page
<${entity}VO> pageList(${entity}Query query) {
    Page<${entity}> page = page(new Page<>(query.getCurrentPage(), query.getCurrentSize()),
    new LambdaQueryWrapper<${entity}>());
    List
    <${entity}VO> resultList = INSTANCE.toVOS(page.getRecords());
        return new Page
        <${entity}VO>().setTotal(page.getTotal()).setRecords(resultList);
            }
            }
            </#if>
