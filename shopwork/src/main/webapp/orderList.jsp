    ...
    <table border="1">
      ...
      <c:forEach items="${list}" var="item">
        <tr>
          ...
          <th>${item.price}</th>
          <th><a href="/shopwork/orderDetail?method=selectPage&order_no=${item.order_no}">查看详细</a>  </th>
        </tr>
      </c:forEach>
    </table>
...
function load(currentPage,pageSize) {
  window.location.href='/shopwork/order?method=selectPage&currentPage='+currentPage+'&pageSize='+pageSize
}
function loadStatus(status) {
  if (status==0) {
    window.location.href='/shopwork/order?method=selectPage'
    return;
  }
  window.location.href='/shopwork/order?method=selectPage&status='+status
}
</script>
</html> 