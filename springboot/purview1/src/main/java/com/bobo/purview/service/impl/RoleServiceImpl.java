...
public class RoleServiceImpl extends ServiceImpl<RoleMapper, Role> implements RoleService {
  ...
  public List<MenuVO> getMenuTreeByRoleId(Integer roleId) {
    //获取全部菜单
    List<Menu> menus = roleMapper.selectMenuTreeByRoleId(roleId, null);
    List<MenuVO> menuVOS = INSTANCE1.toVOS(menus);
    menuVOS.stream()
            .flatMap((Function<MenuVO, Stream<MenuVO>>) menuVO -> {
              List<MenuVO> vos = INSTANCE1.toVOS((List<Menu>) menuVO.getChildren());
              menuVO.setChildren(vos);
              return vos.stream();
            })
            .flatMap((Function<MenuVO, Stream<MenuVO>>) secondLevelMenu -> {
              List<MenuVO> vos = INSTANCE1.toVOS((List<Menu>) secondLevelMenu.getChildren());
              secondLevelMenu.setChildren(vos);
              return vos.stream();
            })
            .forEach(thirdLevelMenu -> {
              List<Permission> permissions = roleMapper.selectPermissionsByRoleIdAndMenuId(roleId, thirdLevelMenu.getMenuId());
              thirdLevelMenu.setChildren(INSTANCE2.toVOS(permissions));
            });
    return menuVOS;
...
//  @Override
//  public List<MenuVO> getMenuTreeByRoleId1(Integer roleId) {
//
//    //获取全部菜单
