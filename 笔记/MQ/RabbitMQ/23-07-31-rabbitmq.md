...
2. 应用解耦
   ...
   ![1690792571193](https://file+.vscode-resource.vscode-cdn.net/e%3A/Visual%20Studio%20Code/java/java_study/%E7%AC%94%E8%AE%B0/MQ/RabbitMQ/image/23-07-31-rabbitmq/1690792571193.png)
3. 异步处理
   有些服务间调用是异步的，例如A 调用B，B 需要花费很长时间执行，但是A需要知道B什么时候可以执行完，以前一般有两种方式，A过一段时间去调用B 的查询 api查询。或者A 提供一个callback apiB执行完之后调用 api 通知A 服务。这两种方式都不是很优雅，使用消息总线，可以很方便解决这个问题A调用B 服务后，只需要监听 B处理完成的消息，当B处理完成后，会发送一条消息给 MQ，MQ 会将此消息转发给A服务。这样A服务既不用循环调用 B 的查询 pi，也不用提供 callback api。同样B 服务也用做这些操作。A 服务还能及时的得到异步处理成功的消息。
   ![1690794017667](image/23-07-31-rabbitmq/1690794017667.png)
...
![1690797730267](image/23-07-31-rabbitmq/1690797730267.png)

## 核心部分

![1690797764373](image/23-07-31-rabbitmq/1690797764373.png)

## RabbitMQ特性

![1691063010055](image/23-07-31-rabbitmq/1691063010055.png)
