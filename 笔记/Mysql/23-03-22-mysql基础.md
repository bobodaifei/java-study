...
1. 脏读：当一个事务**读取另一个事务尚未提交的修改**时，产生脏读。可以读未提交的
2. 不可重复读：在同一事务中多次进行查询，但由于其他**提交**的事务所做的**修改或删除（数据的变化）**，每次返回不同的结果集，此时发生不可重复读。可以读被修改的数据
3. 幻读：在同一事务中多次进行查询，但由于其他**提交**的事务所做的**插入****操作（条数的变化）**，每次返回不同的结果集，此时发生幻读。可以读新数据，但读取不到修改的数据
