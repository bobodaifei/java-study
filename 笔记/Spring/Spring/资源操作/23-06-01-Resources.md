...
![1685597746086](image/23-06-01-Resources/1685597746086.png)

## Resource接口

![1685597930445](image/23-06-01-Resources/1685597930445.png)
...
![1685598030501](image/23-06-01-Resources/1685598030501.png)

**其中一些重要的方法：**

1. getInputStream(): 找到并打开资源，返回一个InputStream以从资源中读取。预计每次调用都会返回一个新
2. InputStream()，调用者有责任关闭每个流
3. exists(): 返回一个布尔值，表明某个资源是否以物理形式存在
4. isOpen: 返回一个布尔值，指示此资源是否具有开放流的句柄。如果为true，InputStream就不能够多次读取，只能读取一次并且及时关闭以避免内存泄漏。对于所有常规资源实现，返回false，但是InputStreamResource除外。
5. getDescription(): 返回资源的描述，用来输出错误的日志。这通常是完全限定的文件名或资源的实际URL。

**其他方法：**

1. isReadable(): 表明资源的目录读取是否通过getInputStream()进行读取。
2. isFile(): 表明这个资源是否代表了一个文件系统的文件。
3. getURL(): 返回一个URL句柄，如果资源不能够被解析为URL，将抛出IOException
4. getURI(): 返回一个资源的URI句柄
5. getFile(): 返回某个文件，如果资源不能够被解析称为绝对路径，将会抛出FileNotFoundException
6. lastModified(): 资源最后一次修改的时间戳
7. createRelative(): 创建此资源的相关资源
8. getFilename(): 资源的文件名是什么 例如：最后一部分的文件名 myfile.txt

## Resource的实现类

![1685598078380](image/23-06-01-Resources/1685598078380.png)
...
![1685600048071](image/23-06-01-Resources/1685600048071.png)

## ResourceLoader接口

![1685600073616](image/23-06-01-Resources/1685600073616.png)
...
另外，使用ApplicationContext访问资源时，可通过不同前缀指定强制使用指定的ClassPathResource、FileSystemResource等**实现类**
...
在 Spring 中，XML 配置文件中使用 `<property>` 标签可以设置 Bean 的属性值，属性值可以是基本类型或者引用类型。在这里，value 属性的值是一个字符串，而且字符串的开头是 classpath:，在 Spring 容器启动的时候，就会将这个值转化为一个 Resource 类型的对象，也就是使用 ClasspathResource 来加载类路径下的资源文件。因此，将这个 Resource 注入到 Bean 的属性中，就能够在后续的代码中调用 Resource 对象提供的方法来读取和操作资源文件的信息了。
